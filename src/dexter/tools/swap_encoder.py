    ...
    try:
        ...
        if recipient == "0xYourWalletAddress":
            ...
            if not private_key:
                return {
                    "success": False,
                    "error": "AGENT_ETH_KEY not found in environment",
                }
            try:
                w3 = Web3()
        ...
        if not token_out_address:
            return {
                "success": False,
                "error": f"Token {token_out} not found in configuration",
            }
        ...
        return {"success": False, "error": f"Failed to encode swap: {str(e)}"}
...
def encode_erc20_approve(
    token: str, spender: str, amount: str
) -> Dict[str, Union[str, bool]]:
    ...
    try:
        ...
        return {
            ...
            "value": "0",
            "function": "approve",
            "params": {"spender": spender, "amount": str(amount_int)},
        }

    except Exception as e:
        return {"success": False, "error": f"Failed to encode approve: {str(e)}"}
