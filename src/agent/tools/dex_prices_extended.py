...
MAVERICK_FACTORY = "0xEb6625D65a0553c9dBc64449e56abFe519bd9c9B"  # Maverick Factory on mainnet
...
def build_curve_route(from_token: str, to_token: str, pool_address: str) -> list:
    ...
    route = [from_address, pool_address, to_address] + ["0x0000000000000000000000000000000000000000"] * 6
    return route[:9]  # Route must be exactly 9 addresses


@tool
def get_curve_price(from_token: str, to_token: str, rpc_url: str = "https://eth.llamarpc.com") -> str:
    ...
    try:
        ...
        if from_token.upper() in ["ETH", "WETH"] and to_token.upper() in ["USDC", "USDT", "DAI"]:
            if to_token.upper() == "USDT":
                pools_to_check.append("tricryptoUSDT")
            elif to_token.upper() == "USDC":
                pools_to_check.append("tricryptoUSDC")
        elif to_token.upper() in ["ETH", "WETH"] and from_token.upper() in ["USDC", "USDT", "DAI"]:
        ...
        if from_token.upper() in ["USDC", "USDT", "DAI"] and to_token.upper() in ["USDC", "USDT", "DAI"]:
            pools_to_check.append("3pool")
        
        # For ETH liquid staking derivatives
        if (from_token.upper() in ["ETH", "WETH", "stETH", "frxETH"] and 
            to_token.upper() in ["ETH", "WETH", "stETH", "frxETH"]):
        ...
        for pool_name in pools_to_check:
            ...
            try:
                ...
                amount_out = router.functions.get_dy(route, swap_params, amount_in).call()
        ...
        if best_price > 0:
            return f"Curve: 1 {from_token} = {best_price:.6f} {to_token} (via {best_pool})"
        else:
...
@tool
def get_fluid_dex_price(from_token: str, to_token: str, rpc_url: str = "https://eth.llamarpc.com") -> str:
...
@tool
def get_maverick_price(from_token: str, to_token: str, rpc_url: str = "https://eth.llamarpc.com") -> str:
